...
def github_url(pkg: Type[spack.package_base.PackageBase]) -> Optional[str]:
    ...
    for repo in spack.repo.PATH.repos:
        ...
        origin_url, toplevel = repo.git_info()
        if not origin_url:
            tty.debug(f"Cannot determine remote origin url for {pkg}, using path URL")
            return path_to_file_url(path)

        # Handle spack repositories cloned with any scheme (e.g., ssh) by
        # ignoring the scheme designation.
        if toplevel and any(name in origin_url for name in ["spack.git", "spack-packages.git"]):
            git_repo = origin_url.split("/")[-1].replace(".git", "").strip()
            prefix = os.path.relpath(os.path.dirname(path), toplevel).replace(os.sep, "/")
            return f"https://github.com/spack/{git_repo}/blob/develop/{prefix}/package.py"
