...
def test_fetch(
    ...
    if type_of_test == "commit":
        s.writable_variants()["commit"] = SingleValuedVariant("commit", t.args["commit"])
...
def test_get_full_repo(
    ...
    if use_commit:
        ...
        s.writable_variants()["commit"] = SingleValuedVariant("commit", commit)
...
def test_commit_variant_clone(git, config, mutable_mock_repo, mock_git_version_info, monkeypatch):
    ...
    monkeypatch.setitem(s.package.versions, Version("git"), args)
    s.writable_variants()["commit"] = SingleValuedVariant("commit", test_commit)
    s.package.do_stage()
    with working_dir(s.package.stage.source_path):
