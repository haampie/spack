...
def gap_truncated_patch(specs: Sequence[Spec]) -> bool:
    """str() abbreviates a patch checksum to seven characters, and two concrete patch values are
    compared for equality instead of by prefix, so a spec naming a patch by its full checksum does
    not round trip."""
    for spec in specs:
        value = spec.variants.get("patches")
        if value is None or not value.concrete:
            continue
        for patch in value.values:
            if isinstance(patch, str) and len(patch) > 7:
                return True
    return False
...
GAPS: Dict[str, Callable[[Sequence[Spec]], bool]] = {
    ...
    "truncated_patch": gap_truncated_patch,
...
LAWS: Tuple[Law, ...] = (
    ...
    Law(
        ...
        gaps=(
            "truncated_patch",
            "dependency_of_a_direct_dependency",
            "nested_direct_edge",
            "propagation",
        ),
    ),
