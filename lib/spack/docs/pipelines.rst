...
by the second job, ``build-pipeline``, and used to trigger the downstream pipeline.
...
The ``repro.json`` file is not versioned and is only designed to work with the version of spack CI that was run with.
