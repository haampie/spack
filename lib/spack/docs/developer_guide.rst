...
:mod:`spack.directory_layout`
  ...
  naming scheme in ``$spack/opt/spack`` (the installation prefix).
...
#. Instantiate a package based on the spec.
...
<major-releases>` (e.g., ``0.21.0``, ``0.22.0``) and :ref:`patch releases
<patch-releases>` (e.g., ``0.22.1``, ``0.22.2``, ``0.22.3``). Here is a
...
tagged version on a release branch. Patch releases (minor releases in older terminology) are backported from
``develop`` onto release branches. This is typically done by cherry-picking
bug fix commits off of ``develop``.
...
Both major and patch releases are tagged. As a convenience, we also tag
...
We schedule work for **major releases** through GitHub `milestones
<https://github.com/spack/spack/milestones>`_ and `Projects
...
#. Create a branch for the release, based on ``develop``:
   ...
   branch if you are preparing the ``vX.Y.0`` release.
...
X (formerly Twitter), Slack, and the mailing list. Here are the steps:

#. Announce the release on X (formerly Twitter).
