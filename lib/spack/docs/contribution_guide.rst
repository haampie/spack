...
there is very little chance that your changes could cause the unit tests to fail.
However, if you make changes to Spack's core libraries, you should run the unit
tests to make sure you did not break anything.

Since they test things like fetching from Git repositories, the unit tests require
Git, `Mercurial <https://www.mercurial-scm.org/>`_,
...
<http://pytest.org/>`_ as our test framework, and these types of
