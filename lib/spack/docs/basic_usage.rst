...
to the new package's prefix.

.. warning::

  The ``spack deprecate`` command makes no promises about binary
  compatibility. It is up to the user to ensure the new package is
...
installed package. The second spec argument is the replacement package's
...
replacement. The ``-I/--no-install-deprecator`` option will ensure
...
the root of the deprecated spec in favor of the root of the replacement
spec.
