...
Spack :ref:`environments <environments>` can easily be turned into container images. This page
outlines two ways in which this can be done:

1. By installing the Spack environment on the host system and copying the installations
...
The first approach is easiest if you already have an installed Spack environment;
...
.. code-block:: console
   ...
   # Create and install a Spack environment in the current directory
   spack env create -d .
...
Spack environment as a separate layer on top. The image is minimal by construction; it only contains the
Spack environment roots and its runtime dependencies.
...
so that future ``spack install`` commands for the Spack environment will simply pull the binaries from the
...
are then pushed both to Docker Hub and to GitHub Container Registry.
The operating systems that are currently supported are summarized in the table below:
...
""""""""""""""""""""""""""""""""""""""""
Use Official Spack Images From Docker Hub
""""""""""""""""""""""""""""""""""""""""
...
to install the built software, all a user has to do is specify:
...
images provided by the vendor (e.g., NVIDIA) and regard CUDA as an external package.
...
Spack environment to explicitly pull it. This requires users to:
...
to your ``mpirun`` calls if started as the root user inside a Docker container.
