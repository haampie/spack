...
A temporary directory to store long-lived cache files, such as indices of
packages available in repositories. It defaults to ``~/.spack/cache`` and can
...
This cache is a subcache of the :ref:`misc-cache-config` and as such will be cleaned when the misc
cache is cleaned.
...
The default location is under the :ref:`misc-cache-config` at: ``$misc_cache/concretization``
